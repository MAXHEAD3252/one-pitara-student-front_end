...
import { AddSections } from "../../modals/create-app-stepper/AddSections";
import { DeleteConfirmationModal } from "../../modals/create-app-stepper/DeleteConfirmationModal";
import { CreateEditSection } from "../../modals/create-app-stepper/CreateEditSection";
...
const TablesWidget49 = () => {
  ...
  const handleShowDeleteModal = (section_id: number) => {
  ...
  const handleModalEdit = (section_id: number, sectionname: string) => {
    setSectionId(section_id);
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${DOMAIN}/api/school/delete-section/${sectionId}`,
        {
          method: "DELETE",
        }
      );
      ...
      toast.error("Failed to delete Section!", { autoClose: 3000 });
    }
  };
            ...
            {filteredData.map((item, index) => (
              ...
              >
                ...
                >
                  <div
                    onClick={() =>
                      handleModalEdit(item.section_id, item.section)
                    }
                    style={{
                      display: "flex",
                      alignItems: "center",
                  ...
                  <div
                    onClick={() => handleShowDeleteModal(item.section_id)}
                    style={{
                      width: "32px",
                      height: "40px",
                      borderRadius: "6px",
